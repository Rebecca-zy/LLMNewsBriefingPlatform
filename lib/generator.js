...
const pickReasoningText = (item) => {
  const candidates = [item.articleSummary, item.articleText, item.description];
...
const buildBilingualIntro = async (item) => {
  const bodyRaw = isUsableSummary(item.articleSummary, item.title) ? normalizeText(item.articleSummary) : "";
...
const generateBriefing = async (options = {}) => {
  ...
  const selected = await Promise.all(
    picked.map(async (item) => {
      ...
      const category = inferNewsCategory(item);
      const enrichedItem = { ...item, category, articleText, articleSummary };
